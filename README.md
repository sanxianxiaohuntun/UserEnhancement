...
# 网页截图配置
webpage_screenshot:
  enable: true              # 是否启用网页功能
  group_enable: true        # 群聊是否启用网页
  private_enable: true      # 私聊是否启用网页
  whitelist_enable: true    # 白名单用户是否启用网页
  timeout: 15               # 网页超时时间(秒)
...
1. 首次使用时会自动创建默认配置文件
2. 网页功能需要Chrome浏览器和ChromeDriver支持
3. 临时文件会在1小时后自动清理
4. 插件卸载时会自动清理所有临时文件
...
| 问题 | 可能原因 | 解决方法 |
|------|---------|---------|
| 网页查看失败 | ChromeDriver版本不匹配 | 更新ChromeDriver至匹配Chrome版本 |
